        ...
        B: 'static,
{
    ...
    fn call(&self, req: ServiceRequest) -> Self::Future {
        ...
        let user = match Claims::from_request_without_body(header) {
            Err(e) => return Box::pin(async move {
                Err(ErrorBadRequest(e))
            }),
            Ok(claims) => claims
        };
        ...
        Box::pin(async move {
            let x = check_user_online(None, user.token_id.clone()).await;
            if !x.is_ok() {
                return Err(ErrorBadRequest(BadRequest::msg("该账户已经退出")));
            }
            ...
            if !CFG.system.super_user.contains(&user.id) {
                ...
                if ApiUtils::is_in(&path).await {
                    if !ApiUtils::check_api_permission(&path, &method, &user.id).await {
                        return Err(ErrorBadRequest(BadRequest::new("你没有权限访问该页面/API", StatusCode::FORBIDDEN)));
                    }
                }
            }
